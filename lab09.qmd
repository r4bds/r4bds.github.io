...
4. Open `R/app_server.R` and add the line `session$onSessionEnded(function() {stopApp()})` as the first line in the `app_server()` function.
  - This will ensure the app closes down when the Shiny server website is refreshed (after waiting ~20 seconds). You do this to ensure that changes you make take effect.
  - If you are working locally, this will not be necessary.
...
4. Double-check that your `app_server()` function in `R/app_server.R` has the line `session$onSessionEnded(function() {stopApp()})`.
5. Look through `dev/run_dev.R` and run the first four commands. If you were not on a server, you could launch your app by running the last command `run_app()`. However, we *are* on a server, so we have implemented a small trick.
  ...
  - Neatly, all your apps will be located here: `https://teaching.healthtech.dtu.dk/shiny/studentID/`
  ...
  - **Another OBS!** For changes to take effect, you will have to refresh the app and wait ~20 seconds.
  ...
  - Refresh your app before continuing so future changes take effect.
...
Note that this will not work on the shiny server, but it is a neat trick when working locally.
...
5. Run every line but the last in `dev/run_dev.R` and reopen your app to see that it works (You can just type in random amino acids in the field).
  ...
  - Remember if you get a "Not Found" error, the "/shiny/" might be missing from the url.
  - If no changes appear, refresh the tab and wait ~20 seconds.
