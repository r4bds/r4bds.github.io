...
Congratulations! You have now created your personal posic cloud account. You will land in `Your Workspace`. Here you will have to create a new project, which you will use to work on the exercises today, so please:
...
1. Then at the menu just above your open file, click `Run App`
...
# Simple base counts
base_freqs <- function(x){
  dna_vec <- strsplit(x = x,
                      split = "")
...
- **Task:** Update your app using the above `card()`-function layout
...
_Note, when we publish our app, each file will be "build" and this will create errors if some functions are not found. Therefore, make sure to include `library("PACKAGE_NAME")` in each files for all needed libraries_
...
Then likely, there is a problem withs paths or libraries, did you remember to start this last part as a seperate project? Also, did you remember to include library definitions in all 3 files?
