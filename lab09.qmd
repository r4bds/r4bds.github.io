...
2. Go back to the RStudio Cloud project.
...
2. Remember to change the project name to `group_X_shiny`. Remember to replace the **X**
3. Install `devtools` and `golem`
...
5. Go back to the RStudio Cloud project
...
3. Look through `dev/03_deploy.R` to get an idea of how to deploy your app either in `shinapps.io`, as the app I showed in class, or on CRAN. No need to run any of it now.
4. Look through `dev/run_dev.R` and run the first four commands. Then run the last command `run_app()`, to launch your, now empty, app. Now you just need to put stuff in it.
...
It may look overly complicated, and perhaps it is for your purpose. But it is a robust framework that makes maintaining even large apps easy. Building a small app is very easy as well.

You are welcome to ignore the `R/run_app.R` and `R/app_config.R` for now.
...
Install it with: `devtools::install_github("rforbiodatascience22/centralDogma")`
...
1. Go to `dev/02_dev.R` and find the function to add a module. For now, set `with_test = FALSE` - you will not be creating tests for the app in this exercise. Remember to name the module something sensible related to the description above instead of just "module2". Run the function.
...
3. While you are there, we will use the tidyverse pipe in the app. As you remember from building a package last week, we need to import that function to use it. Add a `@importFrom magrittr %>%` tag above the server function in `R/app_server.R`. I suggest adding app-wide import tags here, as you then have a single place to put them (you will not be adding more to this app).
4. Run line 18 in `dev/02_dev.R` to see how dependencies are handled automatically.
5. Redo step 2, but for the ui function. Importantly, place it in the second `tabPanel` in `R/app_ui.R`, instead of the "module2". Give it a sensible title. If you made your own layout, place the module where you imagine it should go.
6. **Go back to the module script**. Use my suggested module layout below. If you want, you are more than welcome to design it yourself. See the layout guide [here](https://shiny.posit.co/r/articles/build/layout-guide/) or [cheat sheet](https://rstudio.github.io/cheatsheets/html/shiny.html) for inspiration.
...
7. Run the app to see what it looks like.

8. Add an input field for the `peptide_sequence`, replace the string with the input you are adding. Please try it out first.
...
9. Add an output field for the `plot`, replacing the placeholder string. Please try it out first.
...
3. Go to `dev/02_dev.R` and run the function in line 18 to add `ggplot2` to your package dependencies.

4. Add `@importfrom ggplot2 theme` to the server function description.

5. Run the app to see that it works (You can just type in random letters): Go to `dev/run_dev.R` and run all lines there.
...
5. Before we continue, try to run the app: Go to `dev/run_dev.R` and run all lines there.
...
7. Let's add some inputs (buttons, text, etc.):
    ...
    - Remember to wrap your input/output IDs with `ns()`
...
For "peptide_sequence", I chose a `verbatimTextOutput`. You may not have seen it before; it is in the shiny cheat sheet, but it simply allows for text output in a grey box.
...
The `renderUI` creates a UI field. This function is specific to the `uiOutput`.
`textAreaInput` is a large text input field - suitable for large text inputs, such as DNA.
The `ns()` should always be called when adding input/output fields in modules. Note that it is not necessary when defining outputs, such as `output$DNA`.
The `dna()` adds the DNA strand as the value in the field, this ensures that the updated DNA value is displayed in the input field.

As an added note, notice that the input and output IDs can be the same, as with the `ns("DNA")` ID for both `uiOutput` and `textAreaInput` in this case. I find it handy to name them the same, so I know which input and output interacts.
</details>
</br>
...
6. Regardless if you used my solution or not, **discuss in your group what goes on in my solution**

<details> <summary>Answer </summary> 
With a direct `renderText` on the `input$DNA`, the output will update whenever the input field changes, without causing the nuisance described above, which would be introduced with a `observeEvent` without an associated `actionButton`. Recall `input$DNA` gets updated when `generate_dna` is clicked.
The first test checks that the input is not `NULL`, which it is upon loading the app, causing an error message in the console, but doesn't necessarily crash the app. An output of `NULL` renders nothing.
...
8. Go to `dev/02_dev.R` and run the function in line 18 (it adds dependencies).
9. Run the app and see if it acts as expected.
...
#### Task 3.7 - Deploy your app to shinyapps.io

1. Go to `dev/03_deploy.R`
2. Run lines 30 and 31
3. Go to `app.R` and follow the instructions for deploying your app
4. See that it is deployed
...
1. Deploy the updated app to shinyapps.io.
2. Hand in a pdf with you discussion points and links to your shiny app GitHub page and the shinyapps.io app.
