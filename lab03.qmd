...
From where do we retrieve the data and to where do we write it?

</details>

Run this code block ONCE and then change the chunk header from ` ```{r} ` to ` ```{r echo=TRUE, eval=FALSE} ` (i.e. see the code, but don't run it), this we do, because once you have run this code once, you have downloaded the data and saved it into your data folder. Moving forward we don't want to download the data each time we run this document
...
But... Make sure, that you have run `library("tidyverse")` somewhere in your rmarkdown document. If you get a message saying "No such library"... How would we install a package?
...
Change the chunk header as before
...
_For these exercises I want you to identify what you see in the plot!_
...
- Consult the [Data visualization with ggplot2 cheatsheet](https://www.rstudio.com/resources/cheatsheets/) to see what options you have available
- Feel free to google BUT, if you do, DO NOT simply copy/paste stackoverflow code in, you HAVE to think about what the code does, then modify and adapt to your needs

```{r, out.width = "90%", fig.align = "center", echo=FALSE}
knitr::include_graphics(path = "images/vis_ggplot_components.png")
```
...
- _Hint: See if you can find something online on `geom_smooth()`_ 
...
- _Hint: Think about `fill` and then see if you can find something online on `geom_tile()`, `scale_fill_gradient2` and how to `ggplot rotate axis labels`_ 
...
- _Hint: Here we need to use `geom_density_ridges()`, but which package contains this and how do we install packages? Also we are using a colour scale called `viridis`, but how do we add this? Also, perhaps there are more themes we can use than just `theme_classic()?`_
