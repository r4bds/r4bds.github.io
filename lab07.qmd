...
```bash
user@pupilX:/your/working/directory$
```
...
*Note that when it's the crew's turn to clone the repository, they will not see the `.gitignore` and `lab07_git_exercises.Rproj` listed there (that area will simply be empty).*
...
*Note: regardless of clicking the refresh button, you must have saved the file (Ctrl. + S or clicking the save button) before it appears on the Git tab*
1. Click `Commit`
...
1. Then click `Push`
1. Wait for every team member to Push, then check that you can see the files in the GitHub repo
1. ![](images/pull.png) `Pull` again and check that you can see your teammates' files in your local `git` repo. Open one of them and check that you can see its contents.  
*Note: when opening a file from another team member, you might get an error saying something like "the file is in a restricted path". If so, click the red on/off on the top right of RStudio, then click on "Start New Session" and try again.*
