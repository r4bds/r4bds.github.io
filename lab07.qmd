...
If you do not set `pull.rebase` at all, the first time that your `pull` conflicts with existing commits git will simply ask you whether you want 
to rebase or merge by default. Setting this option here simply avoids the error message from appearing later on.
</details>

4. Login
5. Under `Note`, where it says `DESCRIBE THE TOKEN'S USE CASE`, delete the text and write e.g.`R for Bio Data Science lab 7 git exercises`
6. Under `Expiration`, where it says `30 days`, change that to `90 days`
7. Do not change any other setting, but simply scroll down and click `Generate token`
8. A new `Personal access tokens (classic)` page will appear, stating your personal access token, which starts with `ghp_`, go ahead and copy it
9. Store the `PAT` somewhere save, e.g. in a password management tool. It will serve as your password when connecting to the repository via R.
10. Again in the `Console`, run the below command and enter your `PAT` when prompted for the password:
