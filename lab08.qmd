...
6. Delete `R/hello.R`. It is an example function that will cause issues later if not deleted.

7. Open the `DESCRIPTION` file and write a title for your package, a small description, and add authors.
...
8. Create an MIT license (from the console) `usethis::use_mit_license("Group name")`
9. _If you didn't tick the "Create git repository"_: Run `usethis::use_git()`. _If the "Git" tab is not in the lower left panel_: reopen the R project.
10. <details>
...
2. Run `usethis::use_data(NAME_OF_YOUR_OBJECT, overwrite = TRUE, internal = TRUE)`
  - This will make the data available to your functions,
3. We also want to make the data visible for your users, so they can see which codon table are used.
...
4. You should also describe how the data was produced. You do that in the file that opens after running `usethis::use_data_raw( name = "NAME_OF_YOUR_OBJECT", open = TRUE )`. It might seem a little redundant, but let's do it now for illustrative purposes. 
  - Copy in everything you did (defining the object and adding it as internal and external data) to this file that just opened.
  - This can work as a reference for yourself if something acts up. You can rerun the entire script to make sure everything is as it should be.
...
**OBS** For some unknown reason, this does not work on the current R version (4.3.1). Instead, you can click the "Build" tab next to the "Git" tab and then "install".
After installing and attaching, your manual should pop up. Try printing the object as well to see what it looks like `print(NAME_OF_YOUR_OBJECT)`.
