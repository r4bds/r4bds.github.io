...
- `model_object` is a **new** variable, we are creating inside the `mutate()`-function, which will contain the result of our call to the `map()`-function
...
gravier_clean_aug_long_nested |>
  ...
  # Pull() on tibbles: This pulls out the model_object variable. Note! This is a list, because we nested!
  pull(model_object) |> 
  
  # Pluck() on lists: From the list we got from the last step, we "pluck" the first element
  pluck(1) |>
  
  # The result of pluck, is a model object, upon which we can call the tidy function
  tidy(conf.int = TRUE,
       conf.level = 0.95)
```
